...
print("\nCarga de saldos de tarjetas de credito")
print("------------------------------------")
...
archivo = r'C:\Users\roberto.bravo\Documents\BI\Tarjeta\Maestro\SALDOS.txt'
...
with open(archivo, 'r') as f:
    ...
    cursor = cnxn.cursor()
    cursor.execute('TRUNCATE TABLE BP.SALDOS_TC')
    ...
    for i, linea in enumerate(datos, start=2):
        ...
        comando = f"INSERT INTO BP.SALDOS_TC VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?)"
